function process(input) {
    ...
    // 校验头仓是否为菜鸟仓且整张单据仓库是否一致
    const birdWarehouseMap = new Map();
    if (input.outsourceLineList != null && input.outsourceLineList.length > 0) {
        input.outsourceLineList.forEach(function (value, index) {
            birdWarehouseMap.set(value.warehouseId, value.syncFlag)
        })
    }
    if (!isEmptyStr(input.warehouseId) && isEmptyStr(input.syncFlag)) {
        if (input.outsourceLineList != null && input.outsourceLineList.length > 0) {
            input.outsourceLineList.forEach(function (value, index) {
                if (!input.warehouseId === value.warehouseId) {
                    H0.ExceptionHelper.throwCommonException("头行仓库需保持一致！");
                }
            })
        }

    }
    if (isEmptyStr(input.warehouseId) && birdWarehouseMap != null && birdWarehouseMap.size > 0) {
        birdWarehouseMap.forEach(function (value, index) {
            if (value.syncFlag === 'true') {
                H0.ExceptionHelper.throwCommonException("头行仓库需保持一致！");
            }
        })
    }
    ...
    if (input.isSubmit === 'true') {
        ...
        if (isEmptyStr(input.syncFlag) || (!isEmptyStr(input.syncFlag) && input.syncFlag === 'false')) {
            ...
            if (input.docTypeCode === 'IN') {
                invokePath = "/v1/" + tenantId + "/rest/invoke";
