function process(input) {
    ...
    const queryPath = '/v1/' + tenantId + '/moOperation-oem/getGoingOemOperation?customizeUnitCode=ZOSC.WIP.OPERATIONOEMPLATFORM.LIST.ONGOING.TABLE&page=0&size=-1';
    ...
    if (input.processStatus != null) {
        param.processStatus = input.processStatus[0]
    }
    BASE.Logger.debug('-------------------工序代工平台进行中查询参数转换-------------------{}', param)
    ...
    if (queryRes != null && queryRes.content != null && queryRes.content.length > 0) {
        ...
        BASE.Logger.debug('-------------------工序代工平台进行中未查询到可导出的数据-------------------')
    }
}